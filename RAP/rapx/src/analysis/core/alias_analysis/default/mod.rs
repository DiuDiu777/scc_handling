...
use crate::{
    analysis::{
        Analysis,
        core::alias_analysis::default::{mfp_alias::HierarchicalAnalyzer, st_alias::STAnalyzer},
        graphs::scc::Scc,
    },
    def_id::*,
    utils::source::*,
...
pub struct STAliasAnalyzer<'tcx> {
    pub tcx: TyCtxt<'tcx>,
    pub fn_map: FxHashMap<DefId, MopFnAliasPairs>,
}

impl<'tcx> Analysis for STAliasAnalyzer<'tcx> {
    fn name(&self) -> &'static str {
        "Alias Analysis (MoP)"
    }
...
impl<'tcx> AliasAnalysis for STAliasAnalyzer<'tcx> {
    fn get_fn_alias(&self, def_id: DefId) -> Option<FnAliasPairs> {
        self.fn_map.get(&def_id).cloned().map(Into::into)
    }
...
impl<'tcx> STAliasAnalyzer<'tcx> {
    ...
    fn query_mop(&mut self, def_id: DefId) {
        ...
        if self.tcx.is_mir_available(def_id) {
            let mut mop_graph = MopGraph::new(self.tcx, def_id);
            let mut hierarchical_analyzer = STAnalyzer::new(&mut mop_graph);
            hierarchical_analyzer.run_analysis();
            rap_debug!("Mop graph crated: {}", mop_graph);
            rap_debug!("Search scc components in the graph.");
            // mop_graph.find_scc();
